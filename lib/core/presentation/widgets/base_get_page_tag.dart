import 'package:flutter_getx_clean_architecture/core/presentation/bindings/base_bindings_tag.dart';
import 'package:flutter_getx_clean_architecture/core/presentation/controllers/base_getx_controller.dart';
import 'package:flutter_getx_clean_architecture/core/presentation/widgets/get_page_mixin.dart';
import 'package:get/get.dart';

abstract class BaseGetPageTag<T extends BaseGetxController> extends GetView<T>
    with GetPageMixin {
  BaseGetPageTag({super.key});

  @override
  T get controller => _controller;

  final pageTag = Get.parameters[tagParam];

  late final _controller = Get.find<T>(tag: pageTag);
}
